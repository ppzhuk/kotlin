...
fun IntroduceTypeAliasData.analyze(): IntroduceTypeAliasAnalysisResult {
    ...
    val groupedBrokenReferences = object : MultiMap<TypeReferenceInfo, TypeReferenceInfo>() {
        override fun createMap(): MutableMap<TypeReferenceInfo, MutableCollection<TypeReferenceInfo>> {
    ...
    if (typeParameters.any { it.typeReferenceInfos.any { it.reference.typeElement == originalType } }) {
        return IntroduceTypeAliasAnalysisResult.Error("Type will be rendered unresolved after extraction")
    }

    return IntroduceTypeAliasAnalysisResult.Success(IntroduceTypeAliasDescriptor(this, "", null, typeParameters))
}

fun IntroduceTypeAliasData.isVisibilityApplicable(): Boolean {
    val parent = targetSibling.parent
    return parent is KtClassBody || parent is KtFile
}

fun IntroduceTypeAliasDescriptor.validate(): DataWithConflicts<IntroduceTypeAliasDescriptor> {
    ...
    if (!originalData.isVisibilityApplicable() && visibility != null) {
        conflicts.putValue(originalType, "No visibility is allowed for local type alias")
    }
